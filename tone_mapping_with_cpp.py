...
def process_channel_to_8bit(channel_prod_raw):
        # 1. 飽和邏輯 (Saturation Logic)
        # 如果數值 >= 1.0 (Q16.24)，直接給最大值 255
        # 硬體: Comparator (if val >= 16777216)
        mask_overflow = (channel_prod_raw >= (1 << 24))
        
        # 2. 線性映射 (Linear Mapping for values < 1.0)
        # 公式: Output = (Raw_Value * 255) >> 24
        # 解釋: 
        #   Raw_Value / 2^24 = 真實浮點數 (0~1)
        #   (Raw_Value / 2^24) * 255 = 目標 8-bit 值
        #   這等同於 (Raw_Value * 255) >> 24
        
        # 加上 Rounding (四捨五入): (x + 0.5)
        # 0.5 對應位移前的數值是 (1 << 23)
        ROUNDING = 1 << 23 
        
        val_scaled = (channel_prod_raw * 255) + ROUNDING
        val_shifted = val_scaled >> 24
        
        # 3. 組合結果 (Mux)
        # 如果溢位，取 255；否則取計算結果
        final_val = np.where(mask_overflow, 255, val_shifted)
        
        # 確保型態為 uint8
        return final_val.astype(np.uint8)
...
def local_tone_mapping_lut(Luminance_FILE_PATH, Bmatrix_FILE_PATH, R, G, B, E, lut_data_l=None, lut_data_e=None):
    ...
    R_orig = R * np.power(2, E-124.0)
    G_orig = G * np.power(2, E-124.0)
    B_orig = B * np.power(2, E-124.0)

    max_E = E.max()
    min_E = E.min()
    print(f"E range from {min_E} to {max_E}")
    ...
    ratio_fix = enforce_q_precision(ratio, 12, 21) # 模擬硬體定點數輸出
    ...
    R_fix_raw = np.trunc(R_orig).astype(np.int64)
    G_fix_raw = np.trunc(G_orig).astype(np.int64)
    B_fix_raw = np.trunc(B_orig).astype(np.int64)

    R_raw_Q8_12 = np.clip(R_fix_raw, 0, (1 << 24) - 1)
    G_raw_Q8_12 = np.clip(G_fix_raw, 0, (1 << 24) - 1)
    B_raw_Q8_12 = np.clip(B_fix_raw, 0, (1 << 24) - 1)

    R_final = R_raw_Q8_12 * ratio_fix_raw
    G_final = G_raw_Q8_12 * ratio_fix_raw
    B_final = B_raw_Q8_12 * ratio_fix_raw

    R_out = process_channel_to_8bit(R_final)
    G_out = process_channel_to_8bit(G_final)
    B_out = process_channel_to_8bit(B_final)
